...
class ToolEmail:
    ...
    def __init__(self, subject="主题", sender="database_homework@163.com",
                 password="alisure1234567", name="ALISURE",
                 smtp_server="smtp.163.com", receiver=['562282219@qq.com']):
        ...
        self.root['From'] = self.__format_address('%s <%s>' % (self.__name, self.__sender))
    ...
    def add_attachment_img(self, name, path="", image_type="png"):
        self.__add_attachment(self.__get_path(name, path), "Data", image_type, name)
        pass

    # 添加 文本附件
    def add_attachment_txt(self, name, path=""):
        self.__add_attachment(self.__get_path(name, path), "text", "txt", name)
        pass

    # 添加 附件: 需指定 maintype, subtype
    def add_attachment(self, name, maintype, subtype, path=""):
        self.__add_attachment(self.__get_path(name, path), maintype, subtype, name)
        pass

    # 读取附件
    def __add_attachment(self, file, maintype, subtype, name):
    ...
    # 添加文本附件：add_attachment_txt(name, path=None)
    # 发送：send_email()
